...
public class Hochberg implements IAdjust {
    @Override
    public Map<String, Double> adjust(List<AlgorithmMeans> means, AlgorithmMeans algorithmName, double factor)
    {
        ...
        List<Integer> intOrder = RFunctions.orderReverse(order);

        double[] order2 = new double[order.length];
        for (int j = 0; j < order2.length; j++)
        {
            int transform = order2.length - j;
            order2[j] = (order.length - transform +1)*order[intOrder.get(j)];
        }

        double[] cum = RFunctions.cummin(order2);
